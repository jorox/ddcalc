...
namespace cai{
  ...
  namespace{
    ...
    double special_func_parallel( double mu, double nu, double cw,
                                      Eigen::Vector3d R,
                                      Eigen::Vector3d t,
                                      Eigen::Vector3d b1,
                                      Eigen::Vector3d b2){
      ...
      res = ( b1DotT * b2DotR + b1DotR * b2DotT -
              ( (2-nu) * b1DotT * b2DotT + b1DotB2 ) * tDotR );
      res *= log(Ra + tDotR);
      res += ( (1-nu) * b1DotT * b2DotT + b1DotB2 ) * Ra;
      res -= ( ( b1DotR - tDotR * b1DotT ) * ( b2DotR - tDotR * b2DotT ) / (Ra * Ra - tDotR * tDotR) ) * Ra;
      res += cw * cw *( ( (1+nu) * b1DotT * b2DotT - 2.0 * b1DotB2 ) / ( 2.0 * (Ra * Ra - tDotR * tDotR) ) ) * Ra;
      res *= mu / 4.0 / M_PI / (1 - nu);
      return res;
    }
  }
  ...
  double interaction_energy(double mu, double nu, double cw,
    ...
    double wInt=0;
    if ( abs(t1.dot(t2)) > 0.995 ){
      wInt += special_func_parallel(mu, nu, cw, R24, t1, b1, b2);
      wInt += special_func_parallel(mu, nu, cw, R13, t1, b1, b2);
      wInt -= special_func_parallel(mu, nu, cw, R14, t1, b1, b2);
      wInt -= special_func_parallel(mu, nu, cw, R23, t1, b1, b2);
    }
    else{
